from trinay import Car
from trinay_environment import Env, Path
import random
import numpy as np
...
record_every = 500  # multiple of these episodes are saved
render_every = 2_000  # multiple of these episode will be render
# 0 : 'pedal_gas'
# 1 : 'pedal_reverse'
# 2 : 'pedal_none'
# 3 : 'steer_right'
...
q_table = np.random.uniform(
    low=-10, high=-1, size=(X_samples, Y_Samples, XTE_samples, len(actions)))
...
for episode in tqdm(range(Total_esipodes+1)):
    ...
    if record:
        ...
        avg_reward = sum(
            episodes_rewards[-record_every:])/len(episodes_rewards[-record_every:])
        aggr_episodes_rewards['avg'].append(avg_reward)
        aggr_episodes_rewards['min'].append(
            min(episodes_rewards[-record_every:]))
        aggr_episodes_rewards['max'].append(
            max(episodes_rewards[-record_every:]))
...
plt.plot(aggr_episodes_rewards['ep'],
         aggr_episodes_rewards['avg'], label='average reward')
plt.plot(aggr_episodes_rewards['ep'],
         aggr_episodes_rewards['min'], label='min reward')
plt.plot(aggr_episodes_rewards['ep'],
         aggr_episodes_rewards['max'], label='max reward')
plt.legend()
plt.grid()
plt.subplot(212)
