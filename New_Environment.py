...
class Env:
    ...
    def reset(self):
        ...
        return np.array([self.car.position.x, self.car.position.y, self.heading, 0.0])
    ...
    def render_env(self, FPS_lock=60):
        ...
        txt = f"x={self.car.position.x:.3f} y={self.car.position.y:.3f} vl={self.car.vl:.3f} vr={self.car.vr:.3f} theta={int(math.degrees(self.car.theta))} heading={self.heading:.3f} inclination = {self.inclination:.3f}"
    ...
    def reward(self,cross_track_error):
        #print('Hello',end='|')
        if 0<=cross_track_error<=self.path.path_width/4:
            return +5
        elif self.path.path_width/4<cross_track_error<=self.path.path_width/2:
            return +2
        elif self.path.path_width/2<cross_track_error<=3*self.path.path_width/4:
            #print('Hi',end='|')
            return 0

    def step(self,action,XTE,heading):
        MAX_STEPS_PER_EPISODE = 0
        count=0
        self.car.update(action,self.dt)
        present = self.car.position
        ...
        present_index = np.argmin(spatial.distance.cdist(
            self.trajectory, np.array([[present.x, present.y]])))
        
        self.frame_reward = -0.1
        self.forward_reward = 0
        if present_index>self.previous:
            self.forward_reward = 5
            #print(self.previous, present_index)

        self.previous = present_index

        index = np.argmin(spatial.distance.cdist(self.trajectory, car_position))
        forward = index + 10
        if forward > 499:
            forward = 499
        delx = self.path.x[forward]-self.path.x[index]
        dely = self.path.y[forward]-self.path.y[index]
        dely = -dely

        if delx==0 and dely>=0:
            self.inclination = 90
        elif delx==0 and dely<0:
            self.inclination = 270
        elif delx>0 and dely>=0:
            slope = dely/delx
            self.inclination = math.degrees(math.atan(slope))
        elif delx<=0 and dely<0:
            slope = dely/delx
            self.inclination = 180+math.degrees(math.atan(slope))
        elif delx<=0 and dely>0:
            slope = dely/delx
            self.inclination = 180 + math.degrees(math.atan(slope))
        elif delx>=0 and dely<0:
            slope = dely/delx
            self.inclination = 360 + math.degrees(math.atan(slope))
        
        theta_degrees = math.degrees(self.car.theta)
        self.heading = theta_degrees
        quotient = self.heading//360
        self.heading -= quotient*360

        self.angle_reward = min(abs(360+self.heading-self.inclination),abs(self.inclination-self.heading))
        XTE.append(cross_track_error)
        heading.append(self.angle_reward)
        if self.angle_reward<=10:
            self.angle_reward=5
        elif self.angle_reward<=30:
            self.angle_reward=2
        else:
            self.angle_reward=-8
        if count > MAX_STEPS_PER_EPISODE : 
            done = True
            return np.array([self.car.position.x, self.car.position.y, self.heading, cross_track_error]), -200.0, self.done
        '''if cross_track_error > 20*self.path.path_width:
            self.done = True
            state_arr = np.array(
                [self.car.position.x, self.car.position.y, self.car.heading, cross_track_error])
            reward_float = -200.0
        ...
        if abs(int(math.degrees(self.car.theta))) > (360.0*3.0):
            self.done = True
            state_arr = np.array(
                    [self.car.position.x, self.car.position.y, self.heading, cross_track_error]
                )
            reward_float = -1000.0
            return state_arr, reward_float, self.done
        ...
        if distance_from_end < self.path.path_width/10:
            self.done = True
            state_arr = np.array(
                [self.car.position.x, self.car.position.y, self.heading, max(self.path.map_boundaries[0], self.path.map_boundaries[1])])
            reward_float = 1000.0
            return state_arr, reward_float, self.done
        if not self.done:
            if cross_track_error<=3*self.path.path_width/4:
                state_arr = np.array(
                    [self.car.position.x, self.car.position.y, self.heading, cross_track_error])
                return state_arr, self.reward(cross_track_error)+self.angle_reward+self.forward_reward+self.frame_reward, self.done
            else:
                self.done = True
                state_arr = np.array(
                    [self.car.position.x, self.car.position.y, self.heading, cross_track_error])
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        cords, reward, done =  env.step(action_space[4],XTE,heading)
        ...
        env.close_quit()
    plt.figure()
    plt.plot(XTE, color='g')
    plt.xlabel('Timestep')
    plt.ylabel('XTE')
    plt.title('Variation of cross-track-error')
    plt.grid()
    plt.show()
    plt.figure()
    plt.plot(heading, color='g')
    plt.xlabel('Timestep')
    plt.ylabel('Heading angle error')
    plt.title('Variation of Heading angle error')
    plt.grid()
    plt.show()
